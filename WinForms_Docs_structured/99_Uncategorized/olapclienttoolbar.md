---
title: olapclienttoolbar.md
original_path: c:/workspace/sf11-winform/WinForms_Docs\99_Uncategorized\olapclienttoolbar.md
...
 
  ...
  Create New Report            Creates a new report, clearing the existing report in order to provide the user a new platform to have new deployment based on the existing cube elements. See also .
  ...
  Remove the selected report   Removes the current report from the report list. The button gets disabled if and only if the report list contains one report in it. See also .
  Rename the selected report   The rename option changes the name of the current report as per the user's input.
  Save the current report      The **Save** option stores the current report/reports contained in the report list in a desired location. See also .
