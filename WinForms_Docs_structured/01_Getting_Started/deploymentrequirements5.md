---
title: deploymentrequirements5.md
original_path: c:/workspace/sf11-winform/WinForms_Docs\01_Getting_Started\deploymentrequirements5.md
...
[       - ][This means the Syncfusion assemblies are running in full trust, which is explained in ]{.BodyText1Char}[Default Deployment Pattern]{.UGHyperlink}[. This scenario is fully supported and there are no additional steps necessary]{.BodyText1Char}[.]
...
{border="0"}Note: If you do not want to delete Syncfusion assembly GAC entries, then in Web.config file, remove the Culture, Version and PublicKeyToken attributes used in all [\<][assemblies][\>,\<][httpHandlers][\>] and [\<][handlers][\> ]nodes.
